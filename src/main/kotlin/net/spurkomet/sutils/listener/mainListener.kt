...
fun mainListener() {
    listen<PlayerJoinEvent> {
        it.joinMessage =
            "${col("dark_gray")}[${col("dark_green")}+${col("dark_gray")}] ${col(if (it.player.isOp) "aqua" else "white")}${it.player.name}"
    ...
    listen<PlayerQuitEvent> {
        it.quitMessage =
            "${col("dark_gray")}[${col("dark_red")}-${col("dark_gray")}] ${col(if (it.player.isOp) "aqua" else "withe")}${it.player.name}"
    ...
    listen<EntityDamageEvent> {
        if (it.entityType == EntityType.PLAYER && damageIndicator) {
            if (it.cause != EntityDamageEvent.DamageCause.ENTITY_ATTACK && it.cause != EntityDamageEvent.DamageCause.ENTITY_SWEEP_ATTACK && it.cause != EntityDamageEvent.DamageCause.ENTITY_EXPLOSION && it.cause != EntityDamageEvent.DamageCause.PROJECTILE) {
                broadcast("${prefix()}${col("green")}${it.entity.name} ${col("red")}Schaden bekommen - [${it.finalDamage.roundToLong()}]")
            }
        }
    }
    listen<EntityDamageByEntityEvent> {
        if (it.entityType == EntityType.PLAYER && damageIndicator) {
            broadcast(
                "${prefix()}${col("green")}${it.entity.name} ${col("red")}was attacked by ${col("green")}${it.damager.name}${col("red")} - [${it.finalDamage.toBigDecimal().setScale(4, RoundingMode.FLOOR)}]"
            )
        }
    }
